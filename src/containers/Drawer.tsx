...
export const useCrumb = (crumb: BreadcrumbType) => {
  const crumbRef = useRef(crumb)
  const { addCrumb, removeCrumb } = useContext(DrawerContext)

  useEffect(() => {
    addCrumb(crumbRef.current)

    return () => {
      removeCrumb(crumbRef.current)
    }
  }, [])
}
...
export const Drawer = ({ path, label, children, outletContent = { crumbs: [] } }: DrawerProps) => {
  ...
  const currentCrumb = { label, path: _.last(drawerCrumbs)?.path + '/' + path }
  const [stateCrumbs, setStateCrumbs] = useState(drawerCrumbs.concat(currentCrumb))
  const depth = useRef(drawerCrumbs.length)

  const { pathname } = useLocation()

  const notTopSlide = !pathname.endsWith(path)
  const hasChild = !pathname.startsWith(path) && notTopSlide
