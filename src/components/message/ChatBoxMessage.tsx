...
const MessageOrEditedMessage = observer(
  ({ message, messageVariant, variationIndex, variationIdToEdit, ...props }: MessageProps) => {
    ...
    if (variationIdToEdit === messageVariant.id) {
      return (
        <MessageToEdit key="message-to-edit" message={message} messageVariant={messageVariant} />
      )
    }
...
export const ChatBoxMessage = observer(
  ({ message, ...props }: Omit<MessageProps, 'messageVariant'>) => {
