...
export class ChatViewModel {
  messageViewModelCache = new EntityCache<MessageViewModel>()
  ...
  async fetchMessages() {
    messageTable
      ...
      .then(loadedMessages => loadedMessages.forEach(this.createAndCacheMessageViewModel))
  }

  createAndCacheMessageViewModel = (message: MessageModel) => {
    return this.messageViewModelCache.put(new MessageViewModel(message))
  }
  ...
  async findAndRegenerateResponse() {
    ...
    if (!messageAfterEditedMessage) {
      ...
      botMessageToEdit = this.createAndCacheMessageViewModel(await this.createIncomingMessage())
  ...
  async createAndPushIncomingMessage() {
    ...
    return this.createAndCacheMessageViewModel(incomingMessage)
  }
  ...
  async addUserMessage(content: string = '') {
    ...
    this.createAndCacheMessageViewModel(userMessage)
